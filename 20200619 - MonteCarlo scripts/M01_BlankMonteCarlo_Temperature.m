...
% Temp =  xlsread('Temperature_Trondheim.csv');
...
% MC.Save.file_name = 'Test_hea_dt';

MC.Save.file_name = 'Sim4_17m_DC4';
MC.Save.path_name = ['Results\',mfilename,'\Scenario 1 - 4-axles load\Simulation 04\Sim4_17m_DC4'];
...
%% Two Axle Truck
% Velocity
MC.Var.Veh.Pos.velr = myMC_varDef([10,16,13,1],'nor','Vehicle velocity (m/s)');
MC.Var.Veh.Pos.velb = myMC_varDef([10,16,13,1],'nor','Vehicle velocity (m/s)');

% Body
MC.Var.Veh.Prop.mb = myMC_varDef([15000,16000,15500,10],'nor','Vehicle mass (Kg)');
...
% Starting Position
MC.Var.Veh.Prop.x0_1 = myMC_varDef([-90,-80,-85,3],'nor','starting position 1');
MC.Var.Veh.Prop.x0_2 = myMC_varDef([-200,-190,-195,3],'nor','starting position 2');
...
MC.Var.Veh.Pos.vel = myMC_varDef([50,60,55,1]*0.44704,'nor','Vehicle velocity (m/s)'); % 50-60 mph for trucks
% MC.Var.T = myMC_varDef(Temp,'arr','Temperature (^oC)');
% MC.Var.Veh.Pos.vel = myMC_varDef([8,12,10,1],'nor','Vehicle velocity (m/s)');
...
while MC.Calc.continue == 1
    ...
    rng(MC.Calc.org_seed+ MC.Calc.counter);
     %MC.Calc.org_seed
    % **************************** MISSING ************************************
    %239250042 five axle
...
%% *************************************************************************
    ...
    % A01_VBI_input_and_run_Long_Bridge_Temperature
    % -------------------------------------------------------------------------
    % ----------------------- Variables to Save -------------------------------
