...
% 
%Beam.Prop.E_normal=3.5e10;
%u0=Q+S*Tact+R*(1-erf((Tact-lam)/tu)); % Modification factor
%Beam.Prop.E =u0*Beam.Prop.E_normal;
%
Beam.Prop.E = 3.5e10;
% Density
Beam.Prop.rho = 2500;
...
Beam.Prop.damp_per = 1;         % Damping percentage
...
Beam.BC.vert_stiff = [-1,2.5e10,2.5e10,2.5e10,-1]; Beam.BC.rot_stiff = [0,1,1,1,0]*4.5e9;
...
Beam.Damage.type =1;          % Global damage
Beam.Damage.E.per = [34,30];    % Stiffnes reduction - [Location, Magnitude] of damage in [%]
...
% -- Type --
  %Calc.Profile.type = -1;                             % To load an existing proile
   %     Calc.Profile.Load.file_name ='P03_Profile_Class_A_Long_bridge_New' %GD=2.00E05 'P03_Profile_Class_A'
...
% Veh(veh_num).Prop.ei =   [-4, 0];         % Coordinate of axle (group) to centre of gravity of its corresponding body [m]
% %Veh(veh_num).Pos.vel = -20;            % Speed of vehicle [m/s] (negative value = vehicle moving from right to left)
% %Veh(veh_num).Pos.x0 = 110;             % Initial position of vehicle (from left bridge support)
% Veh(veh_num).Pos.vel = Veh(2).Pos.velr;
% 
% Veh(veh_num).Pos.x0 = -85;             % Initial pos
...
%Veh(veh_num).Pos.x0 = 110;             % Initial position of vehicle (from left bridge support)
Veh(veh_num).Pos.vel = Veh(1).Pos.vel;   % Speed of vehicle [m/s] (negative value = vehicle moving from right to left)
Veh(veh_num).Pos.x0 =  -110;             % Initial pos
...
% ---- End of script ----    
% for i=1:5
%     x(i)=X.MCSol.Run(i).Beam.f(1);
%     y(i)=Y.MCSol.Run(i).Beam.f(1);
% end
% 
% figure()
% histogram(x)
