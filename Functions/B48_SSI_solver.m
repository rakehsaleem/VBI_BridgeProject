...
for veh_num = 1:Veh(1).Event.num_veh
    ...
    Sol.Veh(veh_num).Under.onBeamF(:,1) = ...
        Sol.Veh(veh_num).Wheels.Urel(:,Calc.Solver.t0_ind_beam).*Veh(veh_num).Prop.kTi' + ...
        Sol.Veh(veh_num).Wheels.Vrel(:,Calc.Solver.t0_ind_beam).*Veh(veh_num).Prop.cTi' + ...
    ...
    while continue_while == 1
        ...
        for veh_num = 1:Veh(1).Event.num_veh
            if t<Veh(veh_num).Pos.t_end_ind_beam
                ...
                F_on_Veh = Veh(veh_num).Prop.kTi'.*h + Veh(veh_num).Prop.cTi'.*hd;
                ...
                Sol.Veh(veh_num).Under.onBeamF(:,t_bri+1) = Veh(veh_num).Static.load + ...
                    Sol.Veh(veh_num).Wheels.Urel(:,t+1).*Veh(veh_num).Prop.kTi' + ...
                    Sol.Veh(veh_num).Wheels.Vrel(:,t+1).*Veh(veh_num).Prop.cTi';
        ...
        for veh_num = 1:Veh(1).Event.num_veh
            for wheel_num = 1:Veh(veh_num).Prop.num_wheels
                ...
                Sol.Veh(veh_num).Under.vel(wheel_num,t_bri+1) = ...
                    ...
                    shape_fun_at_x_p*Sol.Beam.U.value_DOFt(Beam.Mesh.Ele.DOF(ele_num,:),t_bri+1)*Veh(veh_num).Pos.vel*is_on_beam;
            end % for wheel_num = 1:Veh(veh_num).Prop.num_wheels
        end % for veh_num = 1:Veh(1).Event.num_veh
