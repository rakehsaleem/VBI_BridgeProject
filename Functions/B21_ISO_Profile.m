...
% Generation of ISO profile from the ISO code's PSD and using the ifft command
% The spatial frequency PSD and parameters should be used
...
if Gd == 'A'      % ISO Class A
    Gd = 0.01e-6 %Gd_limits(1) + diff(Gd_limits([1,2]))*aux1; %0.01e-6
...
% -- Graphical Check --
% spaf_points = [0.01,0.1,1];
% PSD_points = Gd*(spaf_points/spaf_ref).^(-w);
% figure; loglog(ISO_spaf,ISO_PSD)
%     hold on; plot(spaf_points,PSD_points,'r.','MarkerSize',20);
%     xlabel('Saptial Frequency'); ylabel('PSD'); title('Input PSD');
