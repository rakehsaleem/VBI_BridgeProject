...
for veh_num = 1:Veh(1).Event.num_veh
    ...
    Coup.Sol.U(Veh(veh_num).global_ind,1) = Veh(veh_num).SysM.K\(Veh(veh_num).Static.F_vector_no_grav*Calc.Cte.grav)';
...
for veh_num = 1:Veh(1).Event.num_veh
    ...
    % Force vector
    UnCoup.SysM.F(Veh(veh_num).global_ind) = Veh(veh_num).Static.F_vector_no_grav*Calc.Cte.grav;
...
if Calc.Opt.VBI == 1
    ...
    for t = 1:Calc.Solver.num_t-1
        ...
        for veh_num = 1:Veh(1).Event.num_veh
            ...
            kTi = Veh(veh_num).Prop.kTi;
            cTi = Veh(veh_num).Prop.cTi;
            ...
            for wheel_num = 1:Veh(veh_num).Prop.num_wheels
                ...
                if ele_num > 0
                    ...
                    vals_Kg_aux = NN*kTi(wheel_num) + cTi(wheel_num)*Veh(veh_num).Pos.vel*NNp;
                    vals_Cg_aux = NN*cTi(wheel_num);
                    ...
                    OffDiagBlockMat_d = -(shape_fun_at_x_p*N2w)*Veh(veh_num).Pos.vel;
                    ...
                    vals_Kg_aux = (OffDiagBlockMat*kTi(wheel_num) + OffDiagBlockMat_d*cTi(wheel_num))';
                    vals_Kg_aux2 = (OffDiagBlockMat*kTi(wheel_num))';
                    % Addition to Coupled damping matrix
                    vals_Cg_aux = (OffDiagBlockMat*cTi(wheel_num))';
                    ...
                    vals_F = [vals_F; 0*shape_fun_at_x; ...
                        (kTi(wheel_num)*h_path_t_1(wheel_num) + cTi(wheel_num)*hd_path_t_1(wheel_num))*[N2w';-shape_fun_at_x]];
                ...
                elseif ele_num == 0
                    ...
                    % Force vector
                    vals_F = [vals_F; ...
                        (kTi(wheel_num)*h_path_t_1(wheel_num) + cTi(wheel_num)*hd_path_t_1(wheel_num))*N2w'];
