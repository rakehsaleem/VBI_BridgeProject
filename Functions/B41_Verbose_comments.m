...
for i = 1:Beam.BC.supp_num
    if Beam.BC.vert_stiff(i) == -1
        aux1 = 'Fixed';
    elseif Beam.BC.vert_stiff(i) == 0
        aux1 = 'Free';
    else
        aux1 = [num2str(Beam.BC.vert_stiff(i)),' N/m'];
    end % if Beam.BC.vert_stiff(i) == -1
    if Beam.BC.rot_stiff(i) == -1
        aux2 = 'Fixed';
...
if myIsfield(Calc.Profile,{'Opt',1,'movAvg'},1)
    ...
    disp([blanks(8),'for a window of length ',num2str(round(Calc.Profile.Opt.window_L,2))]);
...
for veh_num = 1:Veh(1).Event.num_veh
    ...
    disp([blanks(16),'x0 = ',num2str(Veh(veh_num).Pos.x0),' m']);
    disp([blanks(16),'x_end = ',num2str(Veh(veh_num).Pos.wheels_x(1,end)),' m']);
    disp([blanks(8),'With following velocity:']);
    disp([blanks(16),'v0 = ',num2str(Veh(veh_num).Pos.vel),' m/s']);
...
if any(Calc.Proc.code == [1,2])
    ...
    disp(['Numeric tolerance = ',num2str(Calc.Cte.tol)]);
...
if Calc.Opt.calc_mode_BM == 1
    disp('BM: The average nodal stress is considered');
else
    disp('BM: NO average nodal stress is considered');
end % if Calc.Opt.calc_mode_BM == 1

% Shear calculation mode
if Calc.Opt.calc_mode_Shear == 1
    disp('Shear: The average nodal stress is considered');
else
    disp('Shear: NO average nodal stress is considered');
