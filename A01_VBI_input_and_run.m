...
% -- Damage --
% Beam.Damage.type = 0;                 % No damage (default value)
Beam.Damage.type = 1;                 % 1 element damage
% Beam.Damage.type = 2;                 % Global damage
Beam.Damage.E.per = [25,60];          % Stiffnes reduction - [Location, Magnitude] of damage in [%]
...
% Calc.Profile.Load.file_name ='PA2' %GD=2.00E05 'P03_Profile_Class_A'
Calc.Profile.type = 0;                  % Smooth profile
% Calc.Profile.type = 2;                % ISO random profile 
% Calc.Profile.Info.class = 'B';        % ISO class in text format
% Calc.Profile.Opt.class_var = 1;       % Consider variation 
...
% Calc.Profile.Save.file_name = 'P03_Profile_Class_A_Long_bridge';
...
% Veh(veh_num).Prop.cTi =  [1,1,2,2,2]*1e4;     % Tyre viscous damping [N*s/m]
...
% Veh(veh_num).Prop.ei =   [Veh(2).Prop.Spa, 0];% Coordinate of axle (group) to centre of gravity of its corresponding body [m]
% %Veh(veh_num).Pos.vel = -20;                  % Speed of vehicle [m/s] (negative value = vehicle moving from right to left)
% %Veh(veh_num).Pos.x0 = 110;                   % Initial position of vehicle (from left bridge support)
...
% %Veh(veh_num).Pos.vel = -20;                  % Speed of vehicle [m/s] (negative value = vehicle moving from right to left)
% %Veh(veh_num).Pos.x0 = 110;                   % Initial position of vehicle (from left bridge support)
% Veh(veh_num).Pos.vel = Veh(2).Pos.velr;       % Speed of vehicle [m/s] (negative value = vehicle moving from right to left)
...
Calc.Solver.max_accurate_frq =  512;            % [Hz]
